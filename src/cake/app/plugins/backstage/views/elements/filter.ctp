 ...
 */
	...
	foreach ($this->data as $k => $item)
	{
		...
		else
		{
			...
			if (isset($backstageSettings['customExtraInfo']) && $backstageSettings['customExtraInfo']) {
				echo $this->Jodel->insertModule(
					$modules[$moduleName]['model'],
					array('view', 'backstage_custom', 'extra_info'),
					$item
				);
			}
			else {
			...
			$colspan = 3;
			//More space for more actions!
			if (isset($backstageSettings['extraActions']) && $backstageSettings['extraActions']) {
				$colspan = 6;
				$links .= $this->Jodel->insertModule(
					$modules[$moduleName]['model'], array(
						'view', 'backstage_custom', 'extra_actions'
					), $item
				);
			}
			$links .= $this->Bl->ediv();
			...
			echo $this->Bl->smartTableRowDry(array(
				array(array(), array('escape' => false, 'colspan' => $colspan), $links)
			));
	...
	echo $this->Bl->sdiv(array('class' => 'pagination bottom'));
		echo $this->Paginator->first('<<');
		if ($this->Paginator->hasPrev())
			echo $this->Paginator->prev('<');
		echo $this->Paginator->numbers(array('modulus' => 9, 'separator' => ''));
