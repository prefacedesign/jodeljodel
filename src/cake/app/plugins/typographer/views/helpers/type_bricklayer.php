...
/**
 * Class TypeBricklayerHelper
 *
 * @property JjAuthHelper JjAuth
 */
 ...
 */
	static $tags_that_need_closing_tag = array(
		...
		'textarea','tfoot','th','thead','title','tr','tt','ul','var', 'menu', 'menuItem'
	);
 ...
 * @return string
 */
	function smenu($htmlAttr = array(), $options = array()) {
		$options += array(
			'menuLevel' => 0,
			'writeCaptions' => true,
			'specificClasses' => true,
			'hiddenCaptions' => false,
			'menuLocation' => array(),          //if different from ourLocation, what is the menuLocation
			'depth' => 0,                       //how many additional menu levels will be created
			'close_me' => false,
			'writeUnselectedSubmenus' => true,
			'wrapTag' => 'ul',
			'itemsWrapTag' => 'li',
			'wrapTagOptions' => array()
		);

		/**
		 * @type int $menuLevel
		 * @type int $depth
		 * @type boolean $writeCaptions
		 * @type boolean $specificClasses
		 * @type boolean $hiddenCaptions
		 * @type boolean $close_me
		 * @type boolean $writeUnselectedSubmenus
		 * @type array $menuLocation
		 * @type array $wrapTagOptions
		 * @type string $wrapTag
		 * @type string $itemsWrapTag
		 */
		extract($options);
		...
		if (empty($menuLocation)) {
			$menuLocation = array_slice($ourLocation, 0, $menuLevel);
		}

		for ($i = 0; $i < $menuLevel; $i++) {
			if (isset($sections[$menuLocation[$i]]['subSections'])) {
				$sections = $sections[$menuLocation[$i]]['subSections'];
			}
			else {
				return false;
			}
		...
		foreach($sections as $sectionName => $sectionSettings) {
			...
			if (isset($sectionSettings['permissions'])) {
				$isAllowed = $this->JjAuth->can($sectionSettings['permissions']);
			}

			if ($isAllowed && $sectionSettings['active'] && $sectionSettings['display'])
				$items[] = $this->menuItem(
					array(),
					am(
						compact(
							'menuLocation','sectionName','sectionSettings','writeCaptions','depth',
							'specificClasses','menuLevel','hiddenCaptions'),
						array('wrapTag' => $itemsWrapTag)
					)
				);
		}
		$wrapTagOptions = am(array('close_me' => false), $wrapTagOptions);

		return $this->stag($wrapTag, $htmlAttr, $wrapTagOptions) . implode($items);
	}

	function emenu($htmlAttr = array(), $options = array()) {
 ...
 * @return string
 */
	function smenuItem($htmlAttr = array(), $options = array()) {
		...
		$menuLocation[] = $sectionName;

		$selected = false;
		if (count($menuLocation) <= count($ourLocation)) {
			$selected = true;

			foreach ($menuLocation as $i => $sectName) {
				if ($ourLocation[$i] != $sectName) {
					$selected = false;
					break;
				}
			}
		...
		$out = $this->stag($wrapTag, $htmlAttr);
		$out .= $this->anchor(array(), $anchorOptions, $this->spanDry($content));
	...
	function emenuItem ($htmlAttr = array(), $options = array()) {
		if (empty($options['wrapTag'])) {
			$options['wrapTag'] = 'li';
		}

		return $this->etag($options['wrapTag']);
	}
...
/**
 * Creates a div with `box_container` css class and css classes for size
 *
 * @access public
 * @param $attr Array with HTML attributes
 * @param $options Array with size config. ex. array('size' => array('M' => 12))
 * @return The HTML of <div> tag
 */
...
/**
 * Creates a div with `box` css class and css classes for size
 *
 * @access public
 * @param $attr Array with HTML attributes
 * @param $options Array with size config. ex. array('size' => array('M' => 12, 'g' => -1))
 * @return The HTML of <div> tag
 */
...
/**
 * Creates a simple div with `float-break` css class
 *
 * @access public
 */
 ...
 */
	...
	protected function _expandSettingsArray($settings) {
		...
		//it means it must be a $htmlAtrr only array
		if (!isset($settings[0])) {
			return array($settings, array());
		}
		//it means it must be array($htmlAttr) format
		if (!isset($settings[1])) {
	...
	public function smartTableRow($htmlAttr, $options, $contentCells) {
		...
		foreach($contentCells as $k => $cellParams) {
			...
			if (isset($automaticColumnNumberClasses) && $automaticColumnNumberClasses) {
				$stdColumnSettings = $this->_mergeSettings(
					$stdColumnSettings,
					array('class' => array('col_'. $this->tableStatus['currentColumn']))
				);
			}
 ...
 */
	...
	function __call($n, $args) {
		...
		if (!$just_name && preg_match('/(^[se])([A-Za-z]\w*)/', $n, $matches)) {
			...
			//@todo Add check: are these allowed tags?
			array_unshift($args, $n);
			return $this->dispatchMethod('tag', $args);
		}
		trigger_error('TypeBricklayerHelper::'.$n.'(): Method does not exists.');
	}
}
